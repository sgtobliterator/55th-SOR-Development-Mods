...
// the AO radius). Default (2000) preserves the original jet behaviour.
private _range    = _aoConfig getOrDefault ["planeRange", 2000];
private _maxDist  = _radius + _range;
private _spawnDist = ((_radius + 1500) min _maxDist) max (_radius + 400);
...
for "_i" from 1 to _wpCount do {
    ...
    private _dist  = (_radius + 800 + (random 1200)) min _maxDist;
