...
private _playersOnly = _module getVariable ["FOD_ZA_RQ_PlayersOnly", false];

private _area = _module getVariable ["ObjectArea", [150, 150, 0, false, -1]];
if (is3DEN) then {
	private _size = (_module get3DENAttribute "Size3") select 0;
	_size params [["_a", 150, [0]], ["_b", 150, [0]]];
	...
	case "init": {
		if (_isActivated && isServer) then {
			...
			if (count _syncedUnits == 0) exitWith {
				["[FOD_ZA_ModuleReserveQRF] No units synced to module."] call FOD_ZA_fnc_debugMessage;
			};

			private _groups = [];
			{ _groups pushBackUnique (group _x); } forEach _syncedUnits;

			{ [_x, _radius, _playersOnly] spawn FOD_ZA_fnc_taskReserve; } forEach _groups;

			["[FOD_ZA_ModuleReserveQRF] Holding %1 group(s), %2 units, radius %3m.",
