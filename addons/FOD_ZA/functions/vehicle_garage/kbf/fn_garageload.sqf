...
#include "\FOD_ZA\functions\Vehicle_Garage\kbf\GarageIDC.inc"

#define IDC_TAB_CAR         930
#define IDC_TAB_TANK        931
#define IDC_TAB_HELICOPTER  932
#define IDC_TAB_PLANE       933
#define IDC_TAB_NAVAL       934
#define IDC_TAB_STATIC      935

#define IDCS_LEFT [IDC_TAB_CAR, IDC_TAB_TANK, IDC_TAB_HELICOPTER, IDC_TAB_PLANE, IDC_TAB_NAVAL, IDC_TAB_STATIC]
...
fnc_Garage_load = {
	...
	{
	  ["Vehicles exist in the Area."] call BIS_fnc_guiMessage;
	};
	...
	{
		["Someone opened the garage in the same place."] call BIS_fnc_guiMessage;
	};
...
set_dlconsole = {
	...
	while {true} do {
		...
		if ((missionNameSpace getVariable "BIS_fnc_arsenal_center") != (missionNameSpace getVariable "Garage_Vehicle")) then
		{
			...
			//Spawn button
			_useList = [];
			if (GPs_separateListBySide) then {
				_useList = switch (side player) do {
				  case west; case blufor : {GPs_vehicleList_BLUFOR;};
				  case east; case opfor : {GPs_vehicleList_OPFOR;};
				  case independent; case resistance : {GPs_vehicleList_INDEP;};
				  case civilian : {GPs_vehicleList_CIV;};
					default {[];};
				};
			};
			...
			} forEach (_useList + GPs_vehicleList_All);

			//test: true => disabled
			_vehFindNum = _vehicleList find (toLower _typeVeh); //Check List
			_testWhitelist = (GPs_useListType == 1) && (_vehFindNum == -1);
			_testBlacklist = (GPs_useListType == 2) && (_vehFindNum != -1);

			_vehicleSide = (getNumber (configfile >> "cfgvehicles" >> _typeVeh >> "side")) call BIS_fnc_sideType;
			_testSide = GPs_limitVehicleBySide && (side player != _vehicleSide) && ([civilian, resistance, sideUnknown, sideFriendly] find _vehicleSide == -1);
			_testUAV = ("VehicleAutonomous" in (_typeVeh call BIS_fnc_objectType)) && GPs_uavDisabled;

			_spawnEnable = true;
			_ctrlSpawnButton = _display displayctrl IDC_GARAGEDISPLAY_SPAWNBUTTON;
			//if ( (((GPs_useListType == 1) && (_vehFindNum == -1)) || ((GPs_useListType == 2) && (_vehFindNum != -1))) || ("VehicleAutonomous" in (_typeVeh call BIS_fnc_objectType) && GPs_uavDisabled) ) then {
			if ((_testWhitelist || _testBlacklist) || _testSide || _testUAV) then {
				_ctrlSpawnButton ctrlEnable false;
				_ctrlSpawnButton ctrlSetTooltip "Spawn is Disabled for this Vehicle!";
				_spawnEnable = false;
			} else {
...
*/
				...
				if (_i == 0) then {
					...
					_insigniaPointList ctrlAddEventHandler ["LBSelChanged",{
						(_this select 1) call sel_insignia_point;
					}];
...
fnc_Garage_CreateVehicle = {
	...
	if (_hitpointsDamage findIf {_x != 0} != -1) then {
		//_damageDelays = missionNameSpace getVariable (format ["Garage_DamageDelay_%1", _typeVeh]);
		_damageDelays = missionNameSpace getVariable [format ["Garage_DamageDelay_%1", _typeVeh], [0,0]];
		[_veh, _hitpointsDamage, _damageDelays] spawn kbf_fnc_vehDamage;
	};
	...
	if("VehicleAutonomous" in (_typeVeh call BIS_fnc_objectType)) then
	{
	...
	} else {
		{
			_unitType = switch (side player) do {
			  case west; case blufor : {GPs_bluforCrew;};
			  case east; case opfor : {GPs_opforCrew;};
			  case independent; case resistance : {GPs_independentCrew;};
			  case civilian : {GPs_civilianCrew;};
				default {GPs_otherCrew};
			};
			_aiUnit = (group player) createUnit [_unitType, _point, [], 0, "CAN_COLLIDE"];
