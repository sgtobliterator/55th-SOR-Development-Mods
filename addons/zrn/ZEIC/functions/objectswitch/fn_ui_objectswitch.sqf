...
[format["Passed: Type: %1 - Radius: %2 - Pos: %3", _type, _searchRadius, _pos], "DEBUG"] call PFUNC(misc_logMsg);
...
if (count _replaceType == 0) exitWith { 
	["No objects loaded to replace.","ERROR"] call PFUNC(misc_logMsg);
};
	...
	// Iterate array containing searchable objects.
	{
		...
		if (_index >= 0) exitWith {
			...
			if (isClass (configFile >> "CfgVehicles" >> _replaceClass)) then {
					...
					[format["Object Switch - Invalid Object: %1",_replaceClass], "ERROR"] call PFUNC(misc_logMsg);
				};
		};
	} forEach _searchType;
...
if (count _toReplace == 0) exitWith { 
	["No objects to replace","INFO"] call PFUNC(misc_logMsg);
};
...
[format["Converted %1 Objects", count _toReplace], "INFO"] call PFUNC(misc_logMsg);
