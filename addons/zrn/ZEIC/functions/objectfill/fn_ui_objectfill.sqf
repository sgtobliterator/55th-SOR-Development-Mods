...
_object = missionNamespace getVariable [QPVAR(UiLastObject), objNull];

if (isNull _object) exitWith { ["No suitable object found!", "ERROR"] call PFUNC(misc_logMsg) };
...
_fnc_checkObject = {
	...
	if (_offSet isEqualTo []) then {
		_objBB = 0 boundingBoxReal _objToFill;
		 _offSet = [0];
		[format ["'%1' is not defined, using model 'top'", typeOf _objToFill], "WARNING"] call PFUNC(misc_logMsg);
	};
...
_fnc_attachItem = {
	...
	([_parent] call PFUNC(misc_Vector2Eden)) params ["_xRot", "_yRot", "_zRot"];
	...
	if (is3DEN) then {
		...
		[_obj, _forceDir] call PFUNC(misc_rotateAroundOwnAxisZ);
