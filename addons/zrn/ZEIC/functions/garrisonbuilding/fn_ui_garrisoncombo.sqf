...
// Populate list.
{
	...
	[format["Cfg Found: %1 - %2 - %3", _side, _factName, _catName], "DEBUG"] call PFUNC(misc_logMsg);
	_display lbAdd format["%1 - %2 (%3)", _factName, _catName, _side];
	_display lbSetData [_forEachIndex, format["%1#%2", _factClass, _catClass]];
	if ((missionNamespace getVariable [QPVAR(UiGarrisonFaction),""]) == _factClass && (missionNamespace getVariable [QPVAR(UiGarrisonCategory),""]) == _catClass) then { _factionIndex = _forEachIndex };
