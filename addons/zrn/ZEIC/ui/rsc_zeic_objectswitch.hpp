class Rsc_ZEIC_ObjectSwitch
{
	idd=1701;

	controlsBackground[]=
	{
		QPVAR(OS_Background),
		QPVAR(OS_Frame),
		QPVAR(OS_Text_Title),
		QPVAR(OS_Text_Radius),
		QPVAR(OS_Text_RadiusMeters),
		QPVAR(OS_Text_Type)
	};
	
	controls[]={
		QPVAR(OS_Combo_Type),
		QPVAR(OS_Slider_Radius),
		QPVAR(OS_Button_OK),
		QPVAR(OS_Button_Cancel)
	};

	class PVAR(OS_Background): PVAR(IGUIBack)
	{
		idc = -1;
		x = Q(0.350468 * safezoneW + safezoneX);
		y = Q(0.357 * safezoneH + safezoneY);
		w = Q(0.242344 * safezoneW);
		h = Q(0.209 * safezoneH);
	};
	class PVAR(OS_Frame): PVAR(RscFrame)
	{
		idc = -1;
		x = Q(0.350468 * safezoneW + safezoneX);
		y = Q(0.357 * safezoneH + safezoneY);
		w = Q(0.242344 * safezoneW);
		h = Q(0.209 * safezoneH);
	};
	class PVAR(OS_Text_Title): PVAR(RscText)
	{
		idc = -1;
		text = "Object Switcher"; //--- ToDo: Localize;
		x = Q(0.355625 * safezoneW + safezoneX);
		y = Q(0.357 * safezoneH + safezoneY);
		w = Q(0.0773437 * safezoneW);
		h = Q(0.033 * safezoneH);
	};
	class PVAR(OS_Text_Radius): PVAR(RscText)
	{
		idc = -1;
		text = "Radius"; //--- ToDo: Localize;
		x = Q(0.355625 * safezoneW + safezoneX);
		y = Q(0.456 * safezoneH + safezoneY);
		w = Q(0.0360937 * safezoneW);
		h = Q(0.022 * safezoneH);
	};
	class PVAR(OS_Text_RadiusMeters): PVAR(RscText)
	{
		idc = 1;
		text = "50m"; //--- ToDo: Localize;
		x = Q(0.355625 * safezoneW + safezoneX);
		y = Q(0.476 * safezoneH + safezoneY);
		w = Q(0.0360937 * safezoneW);
		h = Q(0.022 * safezoneH);
	};
	class PVAR(OS_Text_Type): PVAR(RscText)
	{
		idc = -1;
		text = "Convert To"; //--- ToDo: Localize;
		x = Q(0.355625 * safezoneW + safezoneX);
		y = Q(0.412 * safezoneH + safezoneY);
		w = Q(0.0464063 * safezoneW);
		h = Q(0.022 * safezoneH);
	};
	class PVAR(OS_Combo_Type): PVAR(RscCombo)
	{
		idc = 10;
		x = Q(0.407187 * safezoneW + safezoneX);
		y = Q(0.412 * safezoneH + safezoneY);
		w = Q(0.103125 * safezoneW);
		h = Q(0.022 * safezoneH);
		tooltip = "Choose the type of object to convert to.";
		onLoad= "_this spawn {\
			waitUntil { !isNull (_this select 0) };\
			{ (findDisplay 1701 displayCtrl 10) lbAdd _x } forEach ['Desert (Vanilla)', 'Jungle (Apex)', 'Woodland (Contact)'];\
			if (isClass(configFile >> 'CfgPatches' >> 'CUP_Core')) then {\
				{(findDisplay 1701 displayCtrl 10) lbAdd _x } forEach [ 'Desert (CUP)', 'Woodland (CUP)' ];\
				if (isClass(configFile >> 'CfgPatches' >> 'vn_data_f')) then {\
					{(findDisplay 1701 displayCtrl 10) lbAdd _x } forEach [ 'SOG (Brown)', 'SOG (Green)' ];\
				};\
			} else {\
				if (isClass(configFile >> 'CfgPatches' >> 'vn_data_f')) then {\
					{(findDisplay 1701 displayCtrl 10) lbAdd _x } forEach [ 'Invalid (CUP)', 'Invalid (CUP)', 'SOG (Brown)', 'SOG (Green)' ];\
				};\
			};\
			(findDisplay 1701 displayCtrl 10) lbSetCurSel (missionNamespace getVariable ['ZEIC_UiSwitchCombo', 0]);\
		}";
	};
	class PVAR(OS_Slider_Radius): PVAR(RscSlider)
	{
		idc = 20;
		x = Q(0.407187 * safezoneW + safezoneX);
		y = Q(0.456 * safezoneH + safezoneY);
		w = Q(0.154687 * safezoneW);
		h = Q(0.022 * safezoneH);
		tooltip = "Change the radius of the affected area.";
		onSliderPosChanged = "(findDisplay 1701 displayCtrl 1) ctrlSetText format['%1m', round (_this select 1)];";
		onLoad= "_this spawn { waitUntil { !isNull (_this select 0) }; (findDisplay 1701 displayCtrl 20) sliderSetRange [ 1, 250 ]; (findDisplay 1701 displayCtrl 20) sliderSetPosition 50; };";
	};
	class PVAR(OS_Button_OK): PVAR(RscButton)
	{
		idc = -1;
		text = "OK"; //--- ToDo: Localize;
		x = Q(0.463906 * safezoneW + safezoneX);
		y = Q(0.533 * safezoneH + safezoneY);
		w = Q(0.0567187 * safezoneW);
		h = Q(0.022 * safezoneH);
		onButtonClick  = "[ lbCurSel (findDisplay 1701 displayCtrl 10), sliderPosition (findDisplay 1701 displayCtrl 20) ] spawn zeic_fnc_ui_objectSwitch; findDisplay 1701 closeDisplay 1;";
	};
	class PVAR(OS_Button_Cancel): PVAR(RscButton)
	{
		idc = -1;
		text = "Cancel"; //--- ToDo: Localize;
		x = Q(0.530937 * safezoneW + safezoneX);
		y = Q(0.533 * safezoneH + safezoneY);
		w = Q(0.0567187 * safezoneW);
		h = Q(0.022 * safezoneH);
		onButtonClick  = "findDisplay 1701 closeDisplay 2;";
	};	
};
